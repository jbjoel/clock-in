<script lang="ts">
	...
	import { timer, stats, settings } from '$lib/stores/timer';
	...
	// Watch for completion and play sound
	$effect(() => {
...
<div class="app">
	<header class="header">
		<div class="logo">
			<svg class="logo-icon" width="28" height="28" viewBox="0 0 28 28" fill="none">
				...
				<path d="M14 8V14L18 16" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"/>
			</svg>
			<span class="logo-text">Clock In</span>
		</div>
		<button class="settings-btn" onclick={() => settingsOpen = true} aria-label="Open settings">
			<svg width="22" height="22" viewBox="0 0 22 22" fill="none">
				<path d="M11 14C12.6569 14 14 12.6569 14 11C14 9.34315 12.6569 8 11 8C9.34315 8 8 9.34315 8 11C8 12.6569 9.34315 14 11 14Z" stroke="currentColor" stroke-width="1.5"/>
	...
	<main class="main">
		...
		<div class="timer-wrapper">
			<Timer isComplete={timerState.isComplete} />
		</div>
		...
		<div class="controls">
			...
			{:else if timerState.isRunning}
				<button class="btn btn-primary" onclick={handlePause}>
			...
			{:else}
				...
				{#if timerState.remainingSeconds < timerState.selectedDuration * 60}
					<button class="btn btn-secondary" onclick={handleReset}>
						Reset
					</button>
				{/if}
